...
import { compressImage, createPhotoThumbnail } from '../../lib/mediaCompression';
...
export const InlineComposer = forwardRef<InlineComposerHandle, InlineComposerProps>(function InlineComposer(
  ...
  // The lightbox/player always gets a *fresh* object URL fetched on demand,
  // never the same URL the small inline icon is showing — see the
  // thumbnailUrl comment on buildMediaNode for why the two are kept
  // separate. Revoked as soon as the viewer closes, so a full-resolution
  // decode is never held onto for longer than it's actually on screen.
  async function openPreview(kind: MediaKind, id: string) {
    const media = kind === 'photo' ? await mediaStore.getPhoto(id) : await mediaStore.getAudio(id);
    if (!media) return;
    setPreview({ kind, url: URL.createObjectURL(media.blob) });
  }
  ...
  // `thumbnailUrl` is a small (~96px) preview generated by
  // createPhotoThumbnail, NOT the full attached photo — decoding the real
  // full-resolution image just to paint a 22-36px icon is what was
  // exhausting memory on Diary entries with several photos already
  // attached (see mediaCompression.ts). The full photo is only ever
  // fetched again, lazily, via openPreview when the icon is tapped.
  function buildMediaNode(kind: MediaKind, id: string, thumbnailUrl: string | null): HTMLSpanElement {
  ...
  function insertMediaNode(kind: MediaKind, id: string, thumbnailUrl: string | null) {
    const el = editableRef.current;
    if (!el) return;
    el.focus();
    ...
    const wrapper = buildMediaNode(kind, id, thumbnailUrl);
  ...
  async function handlePhotoFile(file: File) {
    setError(null);
    const id = generateId();
    // Downscale + re-encode before it ever reaches IndexedDB — this app has
    // no server, so an uncompressed photo just sits on the user's device
    // storage forever otherwise.
    const compressed = await compressImage(file);
    await mediaStore.savePhoto(id, compressed);
    // The inline icon only ever needs a tiny thumbnail, not the full photo —
    // see buildMediaNode's comment. Track its URL for cleanup on unmount.
    const thumbnailUrl = URL.createObjectURL(await createPhotoThumbnail(compressed));
    objectUrlsRef.current.push(thumbnailUrl);
    insertMediaNode('photo', id, thumbnailUrl);
  }

  async function handleAudioBlob(blob: Blob) {
    const id = generateId();
    await mediaStore.saveAudio(id, blob);
    insertMediaNode('audio', id, null);
  }
    ...
    async function build() {
      ...
      for (const segment of initialContent) {
        ...
        if (segment.type === 'text') {
          ...
          // Fetch the real photo only long enough to derive a tiny thumbnail
          // from it — the full-resolution blob itself is never kept around
          // or given a lasting object URL here. With several photos already
          // on an entry, decoding each one at full size just to paint a
          // ~22px icon is exactly what was exhausting memory (see
          // mediaCompression.ts's THUMBNAIL_MAX_DIMENSION comment).
          const photo = await mediaStore.getPhoto(segment.id);
          if (cancelled) return;
          let url: string | null = null;
          if (photo) {
            url = URL.createObjectURL(await createPhotoThumbnail(photo.blob));
            if (cancelled) {
          ...
          // Audio has no visual icon to render (just a static mic glyph —
          // see buildMediaNode), so there's nothing to fetch until the user
          // actually taps to play it via openPreview.
          el!.appendChild(buildMediaNode(segment.type, segment.id, null));
        }
      }
      updateEmpty();
    }
  ...
  return (
    <div className="inline-composer">
      ...
      {preview &&
        (preview.kind === 'photo' ? (
          <ImageLightbox src={preview.url} onClose={closePreview} />
        ) : (
          <AudioPlayer src={preview.url} onClose={closePreview} />
        ))}
    </div>
  );
});
